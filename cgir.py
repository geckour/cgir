...
class Infrared:
    ...
    def __init__(self, gpio_send=25, gpio_rec=24, codes_path='codes.json'):
        self._pi = pigpio.pi()
    ...
    def send(self, code):
        ...
        space_wids = {}  # Space(待機)波形, key:長さ, value:ID
    ...
    def record(self):
        ...
        self._pi.callback(self.gpio_rec, pigpio.EITHER_EDGE, self._callback)
    ...
    #     tol    : 許容範囲の割合
    @staticmethod
    def _cl(length, target, tol=0.35):
        if target * (1 - tol) < length < target * (1 + tol):
            return True
        else:
            return False
    ...
    #     frames    : バイト列データ
    @staticmethod
    def frames2str(ir_format, frames):
        if ir_format == FORMAT_AEHA:
            s = 'Format AEHA\n'
        elif ir_format == FORMAT_NEC:
            s = 'Format NEC\n'
